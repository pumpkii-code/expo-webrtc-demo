...
import { CallPostData, BaseMessageData, IcePostData } from "@/types/signal_v3";
...
export default function MqttMaster() {
  ...
  const signalingClientV2 = useRef<MqttSignalingClient | null>(null);
    ...
    try {
      ...
      peerConnection.addEventListener('icecandidate', (event) => {
        if (event.candidate && sessionIdRef.current) {
          signalingClientV2.current?.deviceSendIceCandidate(JSON.stringify(event.candidate));
        } else {
  ...
  const connectSignaling = (serverUrl: string) => {
    ...
    const signalingClient = new MqttSignalingClient(serverUrl, deivceIdRef.current);
