...
export default function Viewer({ wsUrl, masterId }: ViewerProps) {
  ...
  const connectSignaling = (serverUrl: string) => {
    ...
    webSocketRef.current.connect({
      ...
      onRegistered: () => {
        console.log('___[VIEWER] 注册成功___peerid', masterId, viewId)
        webSocketRef.current?.connectTo(masterId, viewId);
      },
      onDisconnected: () => {
        setConnected(false);
      },
      ...
      onOffer: async (description) => {
        ...
        try {
          console.log('[VIEWER] 收到 offer SDP:', { description });
          ...
          webSocketRef.current?.sendAnswer(answer.sdp, masterId);
        } catch (err) {
    ...
    pc.current?.addEventListener('icecandidate', (event) => {
      if (event.candidate) {
        webSocketRef.current?.sendCandidate(event.candidate, masterId);
      }
    });
  };
